document.addEventListener('DOMContentLoaded', () => {
  ...
  async function simulateBotReply(query) {
    try {
        ...
        if (response.ok) {
            ...
            appendBotMessage("⚠ " + (data.detail || "Unable to get AI response."));
        }

    } catch (error) {
        console.error(error);
        appendBotMessage("⚠ Unable to connect to RescueAI AI Assistant.");
    }
}
  ...
  // Survival Mode card navigation
  const survivalCard = document.getElementById('survival-mode-card'); // update ID to match your actual element
  const isPagesSubdir = window.location.pathname.includes('/pages/');

  if (survivalCard) {
    survivalCard.addEventListener('click', () => {
      window.location.href = isPagesSubdir ? 'survival.html' : 'pages/survival.html';
    });
  }
