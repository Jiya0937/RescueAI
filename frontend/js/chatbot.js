...
function initDesktopAssistant() {
  ...
  const webcamOverlay = document.getElementById('webcam-overlay-container');
  const btnCloseWebcam = document.getElementById('btn-close-webcam');
  const btnCapturePhoto = document.getElementById('btn-capture-photo');
  const webcamVideo = document.getElementById('webcam-video-feed');
 ...
 if (btnAttach) btnAttach.addEventListener('click', () => openFileChooser(mediaFileInput));
  if (btnCamera) btnCamera.addEventListener('click', () => openWebcamOverlay());

  if (btnCloseWebcam) btnCloseWebcam.addEventListener('click', () => closeWebcamOverlay());

  async function openWebcamOverlay() {
    ...
    try {
      ...
      closeWebcamOverlay();
      openFileChooser(cameraFileInput);
    }
  }
